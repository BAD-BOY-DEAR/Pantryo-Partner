...
class FetchInventory extends Component {
  constructor(props) {
    super(props);
    this.state = {
      partnerProducts: '',
  ...
  fetchAllProductsOfPartnerApi = async () => {
    ...
    if (!partner_id) {
      showToast('Unable to fetch partner id');
      return;
    } else if (!partner_category) {
      showToast('Unable to fetch category');
      return;
    } else {
      const that = this;
      fetch(
                    ...
                    <View style={styles.inventoryTab}>
                      ...
                      <View style={styles.inventoryRow}>
                        <Text style={styles.qty}>
                          {item.product_qty ? item.product_qty : 'No Quantity'}
                          {' ' + item.product_unit}
                        </Text>
                      </View>
                      <View style={{flex: 1}}>
                      ...
                      <View style={styles.btnsSection}>
                        <Switch
                          ...
                          thumbColor={
                            item.product_status == 'In Stock' ? 'green' : 'red'
                          }
